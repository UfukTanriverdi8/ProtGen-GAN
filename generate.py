...
def load_length_distribution(
    dataset_path: str, max_len: int, min_len: int = 30
) -> list[int]:
    lengths = []
    ...
    if not lengths:
        raise ValueError(
            f"No sequence lengths found in range [{min_len}, {max_len}] from {dataset_path}."
        )
    return lengths
...
def load_seed_sequences(seed_file: str, max_len: int, min_len: int = 30) -> list[str]:
    ...
    if not seqs:
        raise ValueError(
            f"No usable seed sequences in range [{min_len}, {max_len}] from {seed_file}."
        )
    return seqs
...
def safe_local_from_pretrained(path: str, device: str):
    ...
    if not os.path.exists(os.path.join(path, "config.json")):
        raise FileNotFoundError(
            f"Missing config.json in {path} (did save_pretrained run?)"
        )
...
# -------------- builders --------------
def build_fully_masked_batch(
    batch_sizes: list[int], cls_id: int, sep_id: int, mask_id: int, pad_id: int, device
...
# -------------- generators --------------
@torch.no_grad()
def generate_full_mode_sequences(
    generator: Generator,
    tokenizer,
    num_sequences: int,
    ...
    while n_done < num_sequences:
        ...
        batch_lengths = [
            sample_sequence_length(length_pool, rng) for _ in range(this_batch)
        ]
        input_ids, attn = build_fully_masked_batch(
            batch_lengths, cls_id, sep_id, mask_id, pad_id, device
        )
        current_rate = 1.0
        ...
        for i in range(this_batch):
            out.append(
                tokenizer.decode(input_ids[i], skip_special_tokens=True).replace(
                    " ", ""
                )
            )
        n_done += this_batch
...
@torch.no_grad()
def generate_seeded_mode_sequences(
    generator: Generator,
    tokenizer,
    num_sequences: int,
    seed_pool: list[str],
    keep_init: float = 0.10,
    keep_percent: float = 0.10,
    temperature: float = 1.0,
    batch_size: int = 64,
    device: str | torch.device = "cuda",
    ...
    out, n_done = [], 0
    max_steps = (
        max(1, math.ceil((1.0 - keep_init) / keep_percent)) + 1
    )  # generous bound
    N = len(seed_pool)
    ...
    while n_done < num_sequences:
        ...
        input_ids, attn = build_seeded_batch(
            batch_seqs,
            tokenizer,
            keep_init,
            cls_id,
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Generate sequences in full & seeded modes."
    )
    ap.add_argument(
        "--ckpt_id",
        required=True,
        help="Trained: 'run/epoch_XX' | Finetuned: '10p_untrained'/'full_untrained' | Base: 'protbert_base'",
    )
    ap.add_argument(
        "--dataset_path",
        type=str,
        default="./data/dnmt_unformatted.txt",
        help="For length sampling (full mode).",
    )
    ap.add_argument(
        "--seed_file",
        type=str,
        default=None,
        help="Held-out seeds file (one seq per line). If None, uses dataset_path.",
    )
    ap.add_argument(
        "--num_full",
        type=int,
        default=5000,
        help="How many full-mode sequences to generate.",
    )
    ap.add_argument(
        "--num_seeded",
        type=int,
        default=5000,
        help="How many seeded-10% sequences to generate.",
    )
    ap.add_argument(
        "--keep_init",
        type=float,
        default=0.10,
        help="Initial kept fraction for seeded mode.",
    )
    ap.add_argument(
        "--keep_percent", type=float, default=0.10, help="Per-iteration fill fraction."
    )
    ...
    ap.add_argument(
        "--tokenizer_dir",
        type=str,
        default=PROTBERT_FINETUNED,
        help="Tokenizer path (defaults to finetuned).",
    )
    ...
    # Full-mode lengths
    lengths = load_length_distribution(
        args.dataset_path, max_len=args.max_len, min_len=args.min_len
    )
