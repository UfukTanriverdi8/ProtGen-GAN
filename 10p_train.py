...
def parse_args():
    ...
    parser.add_argument(
        "--run_name", type=str, default="mn5_default_10p_run", help="W&B run name"
    )
    parser.add_argument(
...
def generate_fakes_for_batch(
    ...
    while current_mask_rate > 0:
        ...
        for i in range(final_input_ids.size(0)):
            ...
            if debug:
                print(
                    f"  Batch element {i}, remaining [MASK]: {mask_count}, mask rate: {mask_rate:.2f}"
                )
